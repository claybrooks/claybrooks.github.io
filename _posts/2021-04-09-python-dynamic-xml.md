...
standards at work mandates APIs must (MUST) be created to read from and write to our configuration files.  Thankfully,
...
The trick to this is overriding two class level attributes: `__getattr__`{:.python} and `__setattr__`{:.python}.  If you
...
`__getattr__`{:.python} is only called after a member lookup fails.
...
`super().__setattr__()`{:.python}.  A list of pre-defined instance members needs to be maintained to avoid entering an
